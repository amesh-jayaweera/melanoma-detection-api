...
def detect_mutations(age,patient_dna,gene_name):
    ...
    if is_within_threshold == False:
        defragmented_patient_dna = defragment_patient_dna(gene,patient_dna)
...
def defragment_patient_dna(gene,patient_dna):
...
def setup_defragmentation_algo(gene,patient_dna):
    rows = 3
    cols = len(gene)
